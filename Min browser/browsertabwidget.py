...
class BrowserTabWidget(QTabWidget):
    ...
    def add_browser_tab(self):
        ...
        title = f'Tab {index + 1}'
        web_engine_view.loadFinished.connect(lambda _, i = index, browser = web_engine_view:
                                    self.setTabText(index, web_engine_view.page().title()))
        ...
        playsound("./windows fax sent.wav",block=False)
    ...
    def handle_tab_close_request(self, index):
        if (index >= 0 and self.count() > 1):
            ...
            playsound("./halo4_windows_trash_03.wav",block=False)
